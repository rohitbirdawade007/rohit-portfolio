...
const ExperienceSection = () => {
  ...
  return (
    <section id="experience" className="py-24 relative overflow-hidden bg-[#F8FAFC]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-14">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.05 }}>
            <h2 className="display-md font-black text-slate-900">
              Professional <span className="text-indigo-600">Timeline</span>
            </h2>
            <p className="mt-2 text-[15px] text-slate-500">Career milestones and engineering contributions.</p>
          </motion.div>
          ...
          {loading
            ? Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="flex gap-8 pl-12 py-6">
                  <div className="flex-1 card p-5 animate-pulse bg-white">
            ...
            : exp.map((e, i) => (
                <motion.div key={e._id} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }}
                  ...
                  <div className="relative z-10 shrink-0 mt-6">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center border border-indigo-200 bg-white text-indigo-600 shadow-sm group-hover:border-indigo-500 transition-colors">
                      <Briefcase size={15} />
                    </div>
                  </div>
                    ...
                    <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
                      <div>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="mono text-[10.5px] font-semibold text-slate-500 uppercase tracking-widest">{e.company}</span>
                        </div>
                        <h3 className="text-[16px] font-bold text-slate-900">{e.role}</h3>
                      </div>
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 border border-indigo-100 text-indigo-700 shrink-0">
                        <Calendar size={12} />
                        <span className="mono text-[10.5px] font-semibold">{e.duration}</span>
                      </div>
                    </div>

                    <p className="text-[13.5px] leading-relaxed border-l-2 border-indigo-200 pl-4 mb-4 text-slate-600">
                      {e.description}
                    </p>
