...
import { Plus, Trash2, Edit, Zap, Cpu, Brain, Layers, Activity } from "lucide-react";
...
const SkillManager = () => {
  ...
  return (
    <div className="space-y-10 pb-12">
      <div className="px-2">
         <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter leading-none mb-4">Skill <span className="text-[#6C63FF]">Inventory</span></h1>
               ...
               <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="space-y-5">
                    <div className="flex justify-between items-center px-1">
                       ...
                       <span className="text-sm font-black text-[#6C63FF]">{formData.proficiency}%</span>
                    </div>
                    <div className="flex items-center gap-6">
                       ...
                       <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${formData.proficiency}%` }} className="h-full bg-[#6C63FF] shadow-[0_0_15px_rgba(14,165,233,0.4)]" />
                       </div>
                    </div>
                  </div>
                 ...
                 <CardContent className="p-8 flex items-center justify-between gap-8">
                   <div className="flex-1">
                     <div className="flex items-center gap-4 mb-4">
                        <div className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-[#6C63FF]/5 group-hover:text-[#6C63FF] transition-colors border border-slate-100">
                           <Cpu size={18} />
                        </div>
                        <div>
                        ...
                        <Badge className="ml-auto bg-[#6C63FF]/5 text-[#6C63FF] border-none px-3 py-1 rounded-full text-[10px] font-black tracking-tighter">{s.proficiency}%</Badge>
                     </div>
                      <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                        <div className="h-full bg-[#6C63FF]/30" style={{ width: `${s.proficiency}%` }} />
                      </div>
                   </div>
                   <div className="flex gap-2">
                     <Button size="icon" variant="ghost" onClick={() => { setIsEditing(true); setCurrentId(s._id); setFormData({name: s.name, description: s.description, proficiency: s.proficiency}); }} className="h-12 w-12 rounded-xl hover:bg-[#6C63FF]/5 text-[#6C63FF] transition-all border border-slate-50"><Edit size={16}/></Button>
